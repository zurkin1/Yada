...
def basic_deconv(mix, pure, gene_list_df):
    gene_list_df.replace(["NaN", 'NaT', 'nan'], np.nan, inplace=True)
    ...
    for cell_type in pure:
        ...
        for mix_col in mix_temp:
            ...
            for gene in cell_genelist:
                ...
                dist += ((column[gene] + 0.000001) / (max_column[gene] + 0.000001)) #* \
...
def dtw_deconv(mix, pure, gene_list_df):
    ...
    for cell_type in pure:
        cell_vals = []
        cell_genelist = gene_list_df[cell_type].dropna().sample(frac=0.25) #0.35
