import numpy as np
import pandas as pd
from Yada.diffexp import gene_diff
from Yada.func import *
# import matplotlib
...
def calc_corr(metric, prop, platform, ens_estimate_wt_2):
    ...
    real_weight = pd.read_csv(relative_path + f'./data/{prop}/labels.csv', index_col=0)
...
if __name__ == '__main__':
    ...
    for metric in ['pathweigh']: #'dtw', 'avg', 'abs', 'basic', 'ks', 'euclid', 'taxi']:
        for file in ['Challenge']: #'10x', 'Abbas', 'BreastBlood', CIBERSORT', 'DeconRNASeq', 'DSA', 'EPIC', 'xCell', 'RatBrain', 'TIMER']:
            print(metric, file, ' ', end="")
            res = run_deconv(relative_path + f'./data/{file}/mix.csv', relative_path + f'./data/{file}/pure.csv', file, metric)
    ...
    result[2:].to_csv(relative_path + './data/result.csv')